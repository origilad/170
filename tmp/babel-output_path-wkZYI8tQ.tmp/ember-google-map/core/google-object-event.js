define('ember-google-map/core/google-object-event', ['exports', 'ember'], function (exports, _ember) {
  /* globals google */
  'use strict';
   ...
   * @constructor
   */
  var GoogleObjectEvent = function GoogleObjectEvent(name, config) {
    this._cfg = {
      name: name,
  ...
  GoogleObjectEvent.prototype.callHandler = function (emberObject) {
    var method,
        target = this._cfg.target || emberObject,
        args;
    ...
    if (method) {
      ...
      _ember['default'].warn(fmt('[google-map] The method `%@` was not found on the target, no action taken.'));
    }
  };
  ...
  GoogleObjectEvent.prototype.link = function (emberObject, googleObject) {
    var name, listener;
    _ember['default'].warn('linking a google object event but it has not been unlinked first', !this._listener);
    if (emberObject && googleObject) {
      this._listener = listener = _ember['default'].run.bind(this, 'callHandler', emberObject);
      name = this._cfg.name;
